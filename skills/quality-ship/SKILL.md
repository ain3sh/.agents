...
- Also check `package.json` scripts for canonical commands (`format`, `fix`, `lint`, `knip`, `test`, `typecheck`).
- Fix any issues found. Re-run until clean.
- For large monorepos with `turbo.json`, prefer `turbo run <task> --filter=...` scoped to affected packages.
