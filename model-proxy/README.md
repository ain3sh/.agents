...
- `config.yaml` exposes `gpt-6-sol-fast` and `gpt-6-luna-fast` as client aliases. Both resolve to their unsuffixed upstream models and add `service_tier: priority`.
...
- Factory's plain `custom:openai://gpt-6-sol` entry is intentionally standard speed and is used by heavy and mission workers. The explicit `-fast` entry is intended for the main model. Luna has only a Fast Factory entry.
