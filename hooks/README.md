...
- **`check_guard.py`**: requires recognized validators to use the no-shell `scripts/run-check <label> [--cwd ...] [--env ...] -- <argv...>` grammar. The Python runner owns cwd, environment, attached execution, complete live stdout/stderr, bounded logs, signal forwarding, and exact exit status. The guard tokenizes with Python's standard shell lexer, denies recognized raw validators and shell composition around the canonical runner, and never reconstructs commands. It is a cooperation guard, not a Bash sandbox. Violations also hard-fail `droid exec` by design; internal hook errors fail open. The validator vocabulary is the `tools` list under `[hooks.pre_tool_use.checks]` in `configs/droid.toml`; structural package/test runners live in the hook. Registered for `PreToolUse` on `Execute`.
