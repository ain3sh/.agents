...
Wait on CI with a backgrounded (`fireAndForget`) `gh pr checks <pr> --watch --fail-fast` (one run: `gh run watch <id> --exit-status`) plus a `*/5` loop that only reads that watcher's log/exit and cancels itself once it exits; triage the first failure, never `sleep`+poll or re-list runs.
